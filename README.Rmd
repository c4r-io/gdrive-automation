...
Both the roadmap and the tracker will contain status indicators for various components / phases of a unit. (The technical limitation here is embedding in the google doc an interactive interface for data that is stored elsewhere.)

There are 4 states for the status, which generally progresses from:

"Not started" -> "Submitted" -> "Under review" -> "Approved"

The below documentation clarifies how to resolve situations where the status for a particular item is different in the roadmap vs. the tracker.
